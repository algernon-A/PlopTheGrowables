...
- Optionally **automatically make plopped growable buildings historical**
- Toggle historical (on/off) for all existing buildings
...
### Make buildings historical when plopped (optional)
Automatically applies historical status to any growable building that is manually placed (plopped) while this setting is active.  This includes placement via Find It, Line Tool, or the dev mode prefab menu.

### Toggle historical status for all existing buildings
This is available via the buttons in the options panel to either add or remove historical status for all buildings currently on the map.  These are only enabled when you're in-game (as otherwise there's no buildings to toggle).
...
This mod has undergone some significant changes for version 1.0, including the removal of the custom building levelling system.  As a result of these changes, this mod is **no longer incompatible** with other mods that modify the building levelling system, including (but not limited to) [Urban Inequality](https://mods.paradoxplaza.com/mods/110245/Windows) and [Realistic Workplaces and Households](https://mods.paradoxplaza.com/mods/87755/Windows).
